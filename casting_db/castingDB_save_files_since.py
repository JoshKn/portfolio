"""Scans the Casting DB media directory for files created after a specific date and saves their metadata to a CSV.
"""
...
#search_dir = r'\\nas-server\farm-settings\99_Test_Footage\Casting_Medienordner'
search_dir = r'\\nas-server\media'
compare_date = datetime.datetime(2024, 6, 19)
black_list = ["Thumbs.db", "FNAM", ".DS_Store"]

output_path = r'\\nas-server\transfer\user'
csv_file = f"castingDB_inventory_since_{compare_date.strftime("%Y_%m_%d")}.csv"
...
print(f"Suche Dateien die nach dem {compare_date.strftime("%d.%m.%Y")} kopiert wurden...")
...
for root, dirs, files in os.walk(search_dir):
    for file in files:
        ...
        try:
            ...
            if copy_date > compare_date and str(file) not in black_list:
